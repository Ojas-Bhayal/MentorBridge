...
app.controller('mentorController', function($scope, $http, $location, csrfStore, formatDateForMySQL, formatDateOnly) {
    $scope.data = {};
    ...
    $scope.loadDashboard = function() {
        $http.get('api/dashboard.php').then(function(response) {
            if (response.data.status === 'success') {
                ...
                // Automatically select the first linked student to make escalation and feedback available.
                if (!$scope.selectedStudent && Array.isArray($scope.data.students) && $scope.data.students.length > 0) {
                    $scope.selectedStudent = $scope.data.students[0];
                }
                $scope.loading = false;
            } else {
                $scope.loading = false;
