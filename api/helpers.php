...
function requireMentorStudentLink(PDO $pdo, int $mentorId, int $studentId): void
{
    ...
    // NEW: Check the explicit linking table instead of transient sessions/appointments
    $stmt = $pdo->prepare('SELECT 1 FROM Mentor_Student WHERE mentor_id = ? AND student_id = ?');
